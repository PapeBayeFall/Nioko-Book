...
if mode == "Comparaison par période":
    ...
    def tracer_barres_annuelles_avec_annotations(df, villes):
        ...
        if battery:
            st.metric(label="Niveau de batterie", value=f"{battery.percent}%")
            st.write(f"🔌 Branché sur secteur : {'Oui' if battery.power_plugged else 'Non'}")
        else:
            st.warning("⚠️ Aucun capteur de batterie détecté sur cet appareil.")
    ...
    def tracer_barres_mensuelles_avec_annotations(df, villes):
        # Fusionner les données de 2023, 2024 et 2025 pour chaque mois
        df_2023 = df[df['Année'] == 2023].groupby('Mois')[villes].sum().reset_index()
        df_2024 = df[df['Année'] == 2024].groupby('Mois')[villes].sum().reset_index()
        df_2025 = df[df['Année'] == 2025].groupby('Mois')[villes].sum().reset_index()

        st.write("### Diagrammes en barres : Comparaison des consommations mensuelles (2023 - 2024 - 2025)")
        
        for ville in villes:
            # Fusionner les consommations des trois années
            df_comparatif = df_2025.copy()
            df_comparatif['Consommation_2024'] = df_2024[ville]
            df_comparatif['Consommation_2023'] = df_2023[ville]
            
            # Tracer le diagramme en barres comparatif entre 2023, 2024 et 2025
            fig = px.bar(
                df_comparatif,
                x='Mois',
                y=[ville, 'Consommation_2024', 'Consommation_2023'],
                title=f"Consommations mensuelles de {ville} en 2023, 2024 et 2025",
                text_auto=True,
                labels={'Mois': 'Mois', 'value': 'Consommation', 'variable': 'Année'},
                color='variable',
                color_discrete_sequence=["palegreen", "Mediumseagreen", "DarkCyan"]
            )
            ...
            fig.update_layout(
                ...
                yaxis_visible=False,
                font=dict(
                    family="Time new roman, sans-serif",  # Police choisie
                    size=16,  # Taille de la police
                    color="Black"  # Couleur de la police
                ),
                title_font=dict(
                    family="Time new roman, sans-serif",  # Police du titre
    ...
    # Fonction 3 : Consommations mensuelles de 2024 avec annotations des évolutions par rapport à 2023
    def tracer_barres_mensuelles_2025_avec_annotations(df, villes):
        # Fusionner les données de 2023 et 2024 pour chaque mois
        df_2024 = df[df['Année'] == 2024].groupby('Mois')[villes].sum().reset_index()
        df_2025 = df[df['Année'] == 2025].groupby('Mois')[villes].sum().reset_index()
    ...
    def tracer_barres_trimestrielles_avec_annotations(df, villes):
            ...
            st.write("### Diagrammes en barres : Consommations trimestrielles avec évolutions (2023-2025)")
            ...
            for ville in villes:
                ...
                fig = px.bar(
                    ...
                    y=ville,
                    color='Année',
                    title=f"Consommation trimestrielle de {ville} (2023-2025)",
    ...
    if uploaded_file is not None:
        # Demander à l'utilisateur de choisir le type de traitement
        choix_traitement = st.radio(
            "Choisissez le type de traitement :", 
            ("Tri des données avec 'Mois-Année'", "Affichage des colonnes spécifiques")
        )

        df = pd.read_excel(uploaded_file)

        # Si l'utilisateur choisit le premier traitement (Mois-Année)
        if choix_traitement == "Tri des données avec 'Mois-Année'":
            st.write("### Aperçu des données et tri des données pour une fiabilité :")
            st.write(df.columns[:])  # Affiche les colonnes récupérées

            # Convertir 'Mois-Année' en datetime et trier
            try:
            ...
            tracer_barres_mensuelles_2025_avec_annotations(df, villes)
            tracer_barres_mensuelles_avec_annotations(df, villes)
        ...
        # Si l'utilisateur choisit le deuxième traitement (Affichage des colonnes spécifiques)
        else:
            df.columns = df.columns.str.strip()  # Nettoyage des noms de colonnes
            st.write("Noms des colonnes du fichier :")

            # Vérifier les colonnes présentes
            colonnes_requises = ["Nom", "2021", "2022", "2023", "2024", "Evolution"]
            
            try:
                mes_colonnes = df[colonnes_requises]

                # Affichage des données
                st.subheader("Tableau des données")
                st.dataframe(mes_colonnes)

                # Sélection des axes pour le graphique
                x_column = st.selectbox("Choisir la colonne pour l'axe X", mes_colonnes.columns)
                y_columns = st.multiselect("Choisir la ou les colonnes pour l'axe Y", mes_colonnes.columns, default=["2024"])
                color_column = st.selectbox("Choisir la colonne pour colorier les barres", mes_colonnes.columns)

                fig_bar = px.bar(
                    mes_colonnes, x=x_column, y=y_columns,
                    title="Consommation énergétique",
                    labels={x_column: "Nom", **{y: f'Consommation énergétique ({y})' for y in y_columns}},
                    color=color_column
                )

                st.plotly_chart(fig_bar)
            except KeyError as e:
                st.error(f"Colonnes manquantes dans le fichier : {e}")


def graphes_series(df, date_debut, date_fin):

    # Vérifier si la colonne 'Date' existe dans les colonnes
    if 'Date' in df.columns:
        # Filtrer le DataFrame selon la plage de dates fournies
        df_filtré = df[(df['Date'] >= pd.Timestamp(date_debut)) & (df['Date'] <= pd.Timestamp(date_fin))]
    else:
        # Si 'Date' n'existe pas, vérifier si c'est l'index
        df_filtré = df[(df.index >= pd.Timestamp(date_debut)) & (df.index <= pd.Timestamp(date_fin))]
    ...
    # Afficher le DataFrame filtré pour débogage
    st.write("Données filtrées :")
    st.dataframe(df_filtré)

    # Appel de la fonction pour récupérer les talons
    Talon_de_nuit, Talon_mode_reduit, Talon_diurne, Talon_le_soir = les_talons(df_filtré)

    # Afficher les talons pour débogage
    st.write("Données par Talon :")
    st.write("Talon de Nuit :", Talon_de_nuit)
    st.write("Talon Mode Réduit :", Talon_mode_reduit)
    st.write("Talon Diurne :", Talon_diurne)
    st.write("Talon Le Soir :", Talon_le_soir)

    # Préparer les données pour le traçage
    data_dict = {
        'Talon de Nuit (00h - 05h)': Talon_de_nuit,
        'Talon Mode Réduit (05h - 08h)': Talon_mode_reduit,
        'Talon Diurne (08h - 21h)': Talon_diurne,
        'Talon Le Soir (21h - 23h)': Talon_le_soir
    }

    # Initialiser la liste pour les graphiques
    seriesOverlaidChart = []

    # Traiter chaque période
    for nom_periode, data in data_dict.items():
        # Vérifier si le DataFrame n'est pas vide
        if not data.empty and 'entree_nom_colonne' in data.columns:
            # Remplir les NaN par 0
            data['entree_nom_colonne'] = data['entree_nom_colonne'].fillna(0)

            # Convertir les données en dictionnaire
            data_dict_series = data[['Date', 'entree_nom_colonne']].reset_index().to_dict('records')
            for entry in data_dict_series:
                entry['Date'] = entry['Date'].strftime('%Y-%m-%d')  # Format YYYY-MM-DD

            # Déterminer les options de style
            color_top = 'rgba(67, 83, 254, 0.7)'  # Couleur par défaut
            color_bottom = 'rgba(67, 83, 254, 0.3)'  # Couleur par défaut
            line_color = 'rgba(67, 83, 254, 1)'  # Couleur par défaut

            # Changer les couleurs pour le Talon de Nuit
            if nom_periode == 'Talon de Nuit (00h - 05h)':
                color_top = 'rgba(255, 192, 0, 0.7)'
                color_bottom = 'rgba(255, 192, 0, 0.3)'
                line_color = 'rgba(255, 192, 0, 1)'

            seriesOverlaidChart.append({
                "type": 'Area',
                "data": data_dict_series,
                "options": {
                    "topColor": color_top,
    ...
    st.subheader("Graphiques des Séries Superposées")
    if seriesOverlaidChart:
        renderLightweightCharts([{
            "chart": {
                "type": "overlaid",
                "title": "Consommation Énergétique par Talon"
            },
            "series": seriesOverlaidChart
        }], 'overlaid')
    else:
        st.warning("Aucune donnée à afficher dans le graphique.")

# Fonction pour afficher les graphiques en Matriciel
def graphe_matrix():
    ...
    # Créer un scatter matrix basé sur les colonnes existantes
    fig_matrix = px.scatter_matrix(
        melted_data_talons,
        color='Période',  
        title='Matrice de dispersion des consommations par période',
        color_discrete_sequence=["DarkCyan", "lightseagreen", "MediumAquamarine", "lightgreen"]
    )
...
# Fonction pour afficher les graphiques en valeur Absolue
def graphe_point():
    les_talons_combined_selection_filled_clean, les_talons_combined_selection, melted_data_talons, colonnes_a_tracer=afficher_graphiques() 
    
    # Création du scatter plot avec Plotly Express
    fig_point = px.scatter(
        les_talons_combined_selection,
        x="Date",
        y="Valeur",  # Utilisation de la colonne Valeur pour l'axe Y
        size="Valeur",  # Taille proportionnelle à la valeur
        color="Période",
        hover_name="Période",
        size_max=60,  # Taille maximale des points
        title="Consommation d'énergie par période",
        log_x=False,  # Assurez-vous que log_x est désactivé si ce n'est pas pertinent
        color_discrete_sequence=["Blue", "Yellow", "MediumSeaGreen", "Red"],  # Discrétisation des couleurs
        hover_data={'Valeur': True,  # Utiliser la colonne formatée pour afficher 'kWh'
                    'Date': True,  # Inclure la date dans les infobulles
                    'Période': True}  # Inclure la période dans les infobulles
    )
...
# Fonction pour afficher les graphiques des sommes journalières en Courbe 
def graphe_courbe_p10():
    global colonne_p10
    cal_debut_dt = pd.to_datetime(cal_debut)  # Conversion de la date en datetime
    cal_fin_dt = pd.to_datetime(cal_fin)  # Conversion de la date en datetime
    all_data = analyse()

    # Vérifier si all_data n'est pas None ou vide
    if all_data is not None and not all_data.empty:
        # Récupérer les périodes uniques et définir une palette de couleurs
        periodes = all_data['Période'].unique()
        couleurs_periodes = ["Blue", "MediumSeaGreen", "Red"]

        # Filtrer les données pour exclure les valeurs non positives
        all_data = all_data[all_data[colonne_p10] > 0]

        # Créer le graphique en ligne avec des options de personnalisation
        fig_line = px.line(
            all_data,
            x='Date',  # Axe des X
            y=colonne_p10,  # Axe des Y
            color='Période',  # Grouper par période
            title=f"Évolution des consommations par période : {colonne_p10} \n"
                  f"du {cal_debut_dt.date()} au {cal_fin_dt.date()}",
            labels={
                'Date': 'Date',
                colonne_p10: 'Consommation en kWh',
                'Période': 'Période'
            },
            color_discrete_sequence=couleurs_periodes[:len(periodes)]  # Couleurs adaptées au nombre de périodes
        )

        # Personnalisation du graphique
        fig_line.update_layout(
            title_font=dict(family="Times New Roman", size=24, color="white"),  # Titre du graphique
            xaxis=dict(
                title="Date",
                title_font=dict(family="Times New Roman", size=20),
            ...
            font=dict(family="Times New Roman", size=14),  # Police générale
            plot_bgcolor="black"  # Fond blanc pour plus de clarté
        )
